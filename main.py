...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi('C:\\Users\\fsenella\\PycharmProjects\\ExtractIm\\ExtractIm.ui', self)

        self.setWindowTitle("ImportIM v1.1")
        ...
        self.table_widget.cellClicked.connect(self.display_image)
    ...
    def valider(self):
        ...
        print('fichier' + self.repertoire_renommer)
        rep_source=os.path.dirname(self.repertoire_renommer)
    ...
    def suivant(self):
        if (self.index_renommer < self.nb_images_renommer-1):
            ...
            print(self.repertoire_renommer)
            self.renommer()
        ...
        if nombre > 0:
            ...
            for result in results:
                ...
                if len(image_files) > 0 and (len(image_files) > self.index_renommer):
                    self.repertoire_renommer = image_files[self.index_renommer]
                    print(image_files[self.index_renommer])
                    self.afficher_image_label(image_files[self.index_renommer])
    ...
    def afficher_image_label(self,image):
        ...
        scaled_pixmap_preview = pixmap.scaled(500, 500, Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def garder(self):
        ...
        # On supprimer la premiere ligne du tableau
        self.supprimer_premiere_ligne()
        print("suppression premiere line lfait")
    ...
    def select_directory(self):
        dir_name = QFileDialog.getExistingDirectory(self, "Sélectionner le répertoire source")
        if dir_name:
            self.label.setText(f"{dir_name}")
            self.analyze_directory(dir_name)

    def select_directory_target(self):
        dir_name_target = QFileDialog.getExistingDirectory(self, "Sélectionner le répertoire destination")
        if dir_name_target:
    ...
    def analyze_directory(self, dir_name):
        ...
        self.table_widget.setHorizontalHeaderLabels(["Name", "Size","Creation Time","Path"])
